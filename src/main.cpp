...
void setup() {
    ...
    percussion_ctrl = new Percussion();
    percussion_ctrl->set_on_percussion_change(updatePercussionEnvelope);
    systems.push_back(drawbars);
...
void updateVibrato(Organ::Vibrato *vib) {
    ...
    if (vib->upper == true) {
        upperVibrato.setMode(vib->mode);
    }
    if (vib->lower == true) {
        lowerVibrato.setMode(vib->mode);
    }
